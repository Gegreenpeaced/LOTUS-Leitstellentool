...
if($_POST['type'] == 3)                                                 // Fahrzeug Typen
{
  if($_POST['send'] == 1337)
  {
    ...
    {
      $cat = $_POST['cat_text'];
      header("location:system.php?msg=15&cat=$cat");
    }
    else {
      header("location:system.php?msg=16");
    }
  }
}
  if($_POST['type'] == 4)                                                 // Fahrzeug Typen
  {
    if($_POST['send'] == 1337)
    {
      include("mysql_config.php");
      ...
      mysqli_select_db($con, "$MYSQL_DB"); //Auswahl der Datenbank
      $prim = uniqid();
      $sql = "INSERT INTO `sys_dest` (`prim`, `dest`, `map`) VALUES ('$prim', '" . $_POST['dest_name'] . "', '" . $_POST['dest_map'] . "')";
      mysqli_query($con, $sql);
      $num = mysqli_affected_rows($con);
      if($num > 0)
      {
        $ziel = $_POST['name'];
        header("location:system.php?msg=17&ziel=$ziel");
      }
      else {
        header("location:system.php?msg=18");
      }
    }
}
?>
